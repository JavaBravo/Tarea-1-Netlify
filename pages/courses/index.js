...
async function setCourses(page, sort, order) {
    console.log('query', sort, order);
    const response = await fetch(`${apiUrl}/courses?page=${page}&sort=${sort}&order=${order}`);
...
function handleSort(selectedSort) {
    window.location.href = `/pages/courses/index.html?sort=${selectedSort}&order=${order}`;
};

function handleOrder(selectedOrder) {
    window.location.href = `/pages/courses/index.html?sort=${sort}&order=${selectedOrder}`;
};
